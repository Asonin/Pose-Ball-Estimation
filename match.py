...
def export_combined(name_list, matches):
...
def export_synced(name_list, matches):
    os.makedirs('out/%s/synced/' % args.prefix, exist_ok=True)
    for cam in name_list:
...
for cam in name_list:
    ...
    i = 0
    mtx, dist = load_intrinsic('params/%s/extrinsics/' % args.scene  + 'cam_%s_extrinsic.npz' % cam)
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret==True:
            i += 1
...
# export_combined(name_list, matches)
export_synced(name_list, matches)