...
def save_image_with_projected_poses(config, images, poses, meta, prefix, cameras, transform):
    basename = os.path.basename(prefix)
    dirname = os.path.join(os.path.dirname(prefix), 'image_with_projected_poses')
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    prefix = os.path.join(dirname, basename)
...
def save_image_with_projected_ball_and_poses(config, batch_image, ball_pos, batch_poses, meta, file_name, colorid):
    batch_image = batch_image.flip(1)
    ...
    for i in range(batch_size):
        ...
        if ball_pos is not None:
            # print(ball_pos)
            ball_2d = project_pose(ball_pos.reshape(1, 3), camera, distort=config.DISTORT_IMAGE)
            ball_2d = affine_transform(ball_2d[0], trans)
            # print("debug", ball_2d)
...
def save_3d_images(config, ball_pos, poses, prefix):
    basename = os.path.basename(prefix)
    dirname = os.path.dirname(prefix)
    ...
    for i in range(batch_size):
        ...
        for n, joint in enumerate(final_pose):
            ...
            for k in eval("LIMBS{}".format(len(joint))):
                ...
                y = [float(joint[k[0], 1]), float(joint[k[1], 1])]
                z = [float(joint[k[0], 2]), float(joint[k[1], 2])]
                ax.plot(x, y, z, c=colors[int(n % 10)], lw=4, marker='o', markerfacecolor='w', markersize=5,
                        markeredgewidth=2)
        if i == 0:
            plt.title('3d pose',fontdict={'weight':'normal','size': 15})
