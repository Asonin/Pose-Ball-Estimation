...
class JDETracker(object):
    def __init__(self, frame_rate=30):
        ...
        self.frame_id = 0
        self.det_thresh = 0.2
        self.buffer_size = int(frame_rate / 30.0 * 200)
    ...
    def update(self, id_feature, dets):
        ...
        dists_coord /= np.linalg.norm(dists_coord, axis=0, keepdims=True)
        # print(dists_coord)

        matches, u_track, u_detection = matching.linear_assignment(dists_coord, thresh=0.2)
        ...
        detections = [detections[i] for i in u_detection]
        dists = matching.coord_distance(unconfirmed, detections, metric='euclidean')
        ...
        self.tracked_stracks, self.lost_stracks = remove_fp_stracks(self.frame_id, self.tracked_stracks, self.lost_stracks)
        # print(self.tracked_stracks)
        # get scores of lost tracks
        output_stracks = [track for track in self.tracked_stracks if track.is_activated]
        # logger.info('===========Frame {}=========='.format(self.frame_id))
...
def remove_fp_stracks(frame_id, stracksa, stracksb):
    fpa, fpb = list(), list()
    for a in range(len(stracksa)):
        timea = stracksa[a].frame_id - stracksa[a].start_frame
        timel = frame_id - stracksa[a].frame_id
        times = stracksa[a].start_frame
        if times > 400 and timea < 20 and timel > 10:
            fpa.append(a)
    for b in range(len(stracksb)):
        timeb = stracksb[b].frame_id - stracksb[b].start_frame
        timel = frame_id - stracksb[b].frame_id
        times = stracksb[b].start_frame
        if times > 400 and timeb < 20 and timel > 10:
            fpb.append(b)
    resa = [t for i, t in enumerate(stracksa) if not i in fpa]
    resb = [t for i, t in enumerate(stracksb) if not i in fpb]
    return resa, resb