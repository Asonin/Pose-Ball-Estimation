...
from ball_detector.utils.general import xyxy2xywh, xywh2xyxy, torch_distributed_zero_first
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # img0 = cv2.fastNlMeansDenoisingColored(img0, None, 5, 5, 7, 21)
            # img0 = cv2.fastNlMeansDenoisingColored(img0, None, 15, 15, 7, 21)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            f = []  # image files
            dir_path = [os.path.join(path, p) for p in os.listdir(path) if os.path.isdir(os.path.join(path, p))]
            path = dir_path
