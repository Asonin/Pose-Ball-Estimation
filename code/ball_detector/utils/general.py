...
from ball_detector.utils.google_utils import gsutil_getsize
from ball_detector.utils.torch_utils import is_parallel, init_torch_seeds
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = torch.cat((box[i].float(), x[i, j + 5, None].float(), j[:, None].float()), 1)
...
def kmean_anchors(path='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from ball_utils.datasets import LoadImagesAndLabels
